...
function solicitarPermisoNotificaciones() {
    if (!('Notification' in window)) {
        mostrarMensaje(mensajeEntradaError, 'Tu navegador no soporta notificaciones.', 0);
        return;
    }

    if (Notification.permission === 'granted') {
        mostrarMensaje(mensajeExito, '🔔 Los recordatorios ya están activados.');
        return;
    }

    if (Notification.permission === 'denied') {
        mostrarMensaje(mensajeEntradaError, 'Los recordatorios están bloqueados en la configuración del navegador.', 0);
        return;
    }

    Notification.requestPermission().then((permiso) => {
        if (permiso === 'granted') {
            mostrarMensaje(mensajeExito, '🔔 Recordatorios activados.');
        } else {
